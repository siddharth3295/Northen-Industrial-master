codeunit 50001 "Mfg Process"
{
    ...
    begin
        ...
        ItemJournalLines.SetRange("Journal Template Name", ToTemplateName);
        ItemJournalLines.SetRange("Journal Batch Name", ToBatchName);
        ItemJournalLines.SetRange("Order Type", ItemJournalLines."Order Type"::Production);
        ItemJournalLines.SetRange("Order No.", ProdOrder."No.");
        ItemJournalLines.SetFilter(Quantity, '<>%1', 0);
        if ItemJournalLines.Findset then begin
            //ItemJnlPostBatch.Run(ItemJnlLine);
            CODEUNIT.Run(CODEUNIT::"Item Jnl.-Post", ItemJournalLines);
        end;
        DeleteJnlLines(ToTemplateName, ToBatchName, ProdOrder."No.", ActualLineNo);
        //ProgressBar.Close();
    ...
    local procedure SetTemplateAndBatchName()
    ...
    var
        ...
        IsHandled: Boolean;
        ReservationEntriesforConsumption: Record "Reservation Entry";
    begin
        with ProdOrderComp do begin
            ...
            if Item."Item Tracking Code" <> '' then begin
                ReservationEntriesforConsumption.Init();
                ReservationEntriesforConsumption."Reservation Status" := ReservationEntriesforConsumption."Reservation Status"::Prospect;
                ReservationEntriesforConsumption.Validate("Item No.", "Item No.");
                ReservationEntriesforConsumption.Validate(Quantity, (-1) * Quantity);
                ReservationEntriesforConsumption.Validate("Location Code", "Location Code");
                ReservationEntriesforConsumption.Validate("Quantity (Base)", (-1) * "Quantity (Base)");
                ReservationEntriesforConsumption.Validate("Source Subtype", ReservationEntriesforConsumption."Source Subtype"::"5");
                ReservationEntriesforConsumption.Validate("Source Type", 83);
                ReservationEntriesforConsumption.Validate("Source ID", ItemJnlLine."Journal Template Name");
                ReservationEntriesforConsumption.Validate("Source Batch Name", ItemJnlLine."Journal Batch Name");
                ReservationEntriesforConsumption.Validate("Source Ref. No.", ItemJnlLine."Line No.");
                ReservationEntriesforConsumption.Validate("Serial No.", "NIC_Sr No.");
                ReservationEntriesforConsumption.Validate("Item Tracking", ReservationEntriesforConsumption."Item Tracking"::"Serial No.");
                ReservationEntriesforConsumption.Insert(true)
            end;
    ...
    var
        ...
        QtyToPost: Decimal;
        IsHandled: Boolean;
        ReservationEntriesforOutput: Record "Reservation Entry";
        ...
        with ProdOrderLine do begin
            ...
            if Item.get(ProdOrderLine."Item No.") then
                if item."Item Tracking Code" <> '' then begin
                    ...
                    ProdOrder.SetRange(Status, ProdOrderLine.Status);
                    if ProdOrder.FindFirst() then;
                    ReservationEntriesforOutput.Init();
                    ReservationEntriesforOutput."Reservation Status" := ReservationEntriesforOutput."Reservation Status"::Prospect;
                    ReservationEntriesforOutput.Validate("Item No.", "Item No.");
                    ReservationEntriesforOutput.Validate(Quantity, Quantity);
                    ReservationEntriesforOutput.Validate("Location Code", "Location Code");
                    ReservationEntriesforOutput.Validate("Quantity (Base)", "Quantity (Base)");
                    ReservationEntriesforOutput.Validate(Positive, true);
                    ReservationEntriesforOutput.Validate("Source Subtype", ReservationEntriesforOutput."Source Subtype"::"6");
                    ReservationEntriesforOutput.Validate("Source Type", 83);
                    ReservationEntriesforOutput.Validate("Source ID", ItemJnlLine."Journal Template Name");
                    ReservationEntriesforOutput.Validate("Source Batch Name", ItemJnlLine."Journal Batch Name");
                    ReservationEntriesforOutput.Validate("Source Ref. No.", ItemJnlLine."Line No.");
                    ReservationEntriesforOutput.Validate("Serial No.", ProdOrder."NIC_Sr No.");
                    ReservationEntriesforOutput.Validate("Item Tracking", ReservationEntriesforOutput."Item Tracking"::"Serial No.");
                    ReservationEntriesforOutput.Insert(true);
                end;
